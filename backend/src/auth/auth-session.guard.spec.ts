...
describe("AuthSessionGuard", () => {
  it("verifies bearer tokens and attaches server identity", async () => {
    const auth = {
      authenticate: jest.fn().mockResolvedValue(principal)
    } as unknown as AuthService;
    const tokens = new SessionTokenService(config);
    const guard = new AuthSessionGuard(auth, tokens);
  ...
  it("rejects missing and malformed bearer credentials", async () => {
    ...
    const guard = new AuthSessionGuard(auth, new SessionTokenService(config));

    await expect(guard.canActivate(createContext(undefined).context)).rejects.toMatchObject({
      code: "UNAUTHORIZED",
      statusCode: 401
    });
    await expect(
